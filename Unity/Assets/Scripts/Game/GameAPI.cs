...
{
    private string _baseUrl = "http://localhost:4000/api";                  // Node.js 서버 URL
    ...
    public IEnumerator RegisterPlayer(string playerName, string password)
    {
        ...
        using (UnityWebRequest request = new UnityWebRequest($"{_baseUrl}/register", "POST"))
        {
    ...
    public IEnumerator LoginPlayer(string playerName, string password, Action<PlayerModel> onSuccess)
    {
        ...
        using (UnityWebRequest request = new UnityWebRequest($"{_baseUrl}/login", "POST"))
        {
            byte[] bodyRaw = Encoding.UTF8.GetBytes(jsonData);
            request.uploadHandler = new UploadHandlerRaw(bodyRaw);
            request.downloadHandler = new DownloadHandlerBuffer();
            ...
            else
            {
                ...
                string responesBody = request.downloadHandler.text;

                try
                {
                    var responseData = JsonConvert.DeserializeObject<Dictionary<string, object>>(responesBody);
