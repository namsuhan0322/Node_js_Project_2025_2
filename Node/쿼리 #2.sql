...
-- 실습
-- 1. 새로운 아이템 추가
INSERT INTO items(NAME, DESCRIPTION, VALUE) VALUES ('지팡이', '기본 지팡이 무기', 20)

-- 2. 특정 플레이어의 인벤토리에 새 아이템 추가
INSERT INTO inventories(player_id, item_id, quantity) VALUES (3, 4, 2)

-- 3. 가장 가치 있는 아이템 찾기 (ORDER BY value DESC)
SELECT * FROM inventories ORDER BY quantity DESC
...
-- 1. 새로운 퀘스트 추가
INSERT INTO quests(title, DESCRIPTION, reward_exp, reward_item_id) VALUES
('전설의 무기 획득', '전설의 지팡이를 획득 하세요', 1000, 4)
...
SELECT q.title, q.reward_exp FROM quests q ORDER BY q.reward_exp DESC LIMIT 1